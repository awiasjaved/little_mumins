...
Ameen`
      ...
      
    {
      ...
      image: Jannat,
      hoverImage: Annat,
      description: `“راہ جنت کی کہانی” ایک منفرد کہانی کی کتاب ہے جس کا مقصد بچوں کو دینی تصورات کے ذریعے تعلیم دینا ہے
      <br/>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 max-w-6xl mx-auto px-4">
          {products.map((product, index) => (
            ...
            >
              ...
              <h4 className="text-lg font-medium">{product?.page}</h4>
              <h4 className="text-lg font-medium">{product?.cloth}</h4>
              <h4 className="text-lg font-medium">{product?.size}</h4>

              {/* Product Description with HTML rendering */}
              <p className="text-sm text-gray-500 mt-2" dangerouslySetInnerHTML={{ __html: product?.description }} />
              ...
              <button className="btn mt-4 text-white py-2 px-4 rounded-full transition">
                Add to Cart
              </button>
            </div>
          ))}
